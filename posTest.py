...
def land_on_elevated_surface():
    with SyncCrazyflie(uri, cf=Crazyflie(rw_cache='./cache')) as scf:
        with PositionHlCommander(scf, default_height=0.5, default_velocity=0.2, default_landing_height=0.35) as pc:
            ...
            pc.forward(1.0)
            pc.left(1.0)
            # land() will be called on context exit, gradually lowering to default_landing_height, then stoppig motors
        ...
        with PositionHlCommander(scf) as pc:
            ...
            while True:
                if keyboard.is_pressed('up'):
                    print("loop broken")
                    pc.go_to(0.0, 0.0, 0.0, velocity=0.2)
                    break

                elif keyboard.is_pressed('space'):
                    x_co = -1*(float(input("enter new x coordinate: ")))
                    y_co = float(input("enter new y coordinate: "))
                    z_co = float(input("enter new z coordinate: "))
                elif keyboard.is_pressed('down'):
                    print("loop broken quickly")
                    break
                else:
                    pc.go_to(y_co, x_co, z_co, velocity=0.2)
